...
export class ValidationMiddleware {
  ...
  public static sanitizeInput(
    ...
    if (req.body && typeof req.body === 'object') {
      const sanitizedBody = ValidationMiddleware.deepSanitize(req.body)
    ...
    if (req.query && typeof req.query === 'object') {
      const sanitizedQuery = ValidationMiddleware.deepSanitize(req.query)
      // req.query is often a getter, so we can't reassign it.
      // But we can mutate it if it's mutable, or we might need to be careful.
      // Express usually allows mutation of properties.
      // However, to be safe and fix the "Cannot set property query" error:
      // We should NOT try to set req.query = ...
      // Instead, we can try to mutate the keys.
      // But req.query might be read-only in some setups.
      // Let's try to just sanitize the values if possible, or skip if it fails.

      // Better approach:
      // If we can't reassign req.query, we can try to copy properties back.
      try {
        const keys = Object.keys(req.query)
        keys.forEach(key => {
          // @ts-ignore
          req.query[key] = ValidationMiddleware.deepSanitize(req.query[key])
        })
      } catch (e) {
        // If we can't mutate, we ignore.
        console.warn('Could not sanitize req.query', e)
      }
    }
    ...
    if (req.params && typeof req.params === 'object') {
      try {
        const sanitizedParams = ValidationMiddleware.deepSanitize(req.params)
        Object.keys(req.params).forEach(key => delete req.params[key])
        Object.assign(req.params, sanitizedParams)
      } catch (e) {
        console.warn('Could not sanitize req.params', e)
      }
    }
  ...
  private static getObjectDepth(obj: any, currentDepth: number = 0): number {
    ...
    for (const key in obj) {
      if (Object.prototype.hasOwnProperty.call(obj, key)) {
        const depth = ValidationMiddleware.getObjectDepth(
          obj[key],
          currentDepth + 1
        )
  ...
  public static validateArrayLimits(maxLength: number = 100) {
    return (req: Request, res: Response, next: NextFunction): void => {
      ...
      const hasLargeArray = ValidationMiddleware.checkArraySizes(
        req.body,
        maxLength
      )
  ...
  private static checkArraySizes(obj: any, maxLength: number): boolean {
    if (Array.isArray(obj)) {
      ...
      return obj.some(item =>
        ValidationMiddleware.checkArraySizes(item, maxLength)
      )
    }

    if (obj && typeof obj === 'object') {
      return Object.values(obj).some(value =>
        ValidationMiddleware.checkArraySizes(value, maxLength)
      )
    }
   ...
   */
  public static validateEncoding(
    req: Request,
    res: Response,
    next: NextFunction
  ): void {
